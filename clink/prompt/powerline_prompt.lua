...
local segment = {
    isNeeded = true,
    text = "",
    textColor = colorBlackBr, 
    fillColor = colorBlueBr
}
...
local function init()
    ...
    -- This helps users avoid having a super long prompt
	    homenocase = nocase(clink.get_env("USERPROFILE"))
        if plc_prompt_useHomeSymbol and string.find(cwd, homenocase) then 
            -- in both smart and full if we are in home, behave like a proper command line
            cwd = string.gsub(cwd, homenocase, plc_prompt_homeSymbol)
        else 
	...
	if plc_prompt_useDirSymbol then
		cwd = string.gsub(cwd,"\\",plc_prompt_dirSymbol)
	end
